...
public class SQLArticleDAO implements NewsDAO {
	...
	private static final String SELECT_LAST_ARTICLES_QUERY = "SELECT * FROM news where status = ? ORDER BY id DESC LIMIT ?\r\n";
	private static final String SELECT_PAGED_ARTICLES_QUERY = 	"SELECT * FROM news where status = ? ORDER BY id DESC LIMIT ? , ?";
	...
	public List<Article> getLast(int quantity) throws DAOException {
		...
		try (Connection connection = connectionPool.takeConnection();
				...
				PreparedStatement stmt = connection
						.prepareStatement(SELECT_LAST_ARTICLES_QUERY)) {
			stmt.setString(1, ACTIVE);
			...
			e.printStackTrace();
			throw new DAOException("getting last articles error", e);
		}
	...
	public List<Article> getNewsPaged(int quantity, int page) throws DAOException {
		...
		try (Connection connection = connectionPool.takeConnection();
				...
				PreparedStatement stmt = connection
						.prepareStatement(SELECT_PAGED_ARTICLES_QUERY)) {
			stmt.setString(1, ACTIVE);
			stmt.setInt(2, gap);
